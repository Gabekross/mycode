...
def main():
    ...
    cards = resp.json()
    with open("mtgsetscustom.index", "w") as mtgfile:
        for card in cards:  # loop across ALL of the sets of MTG cards (they are stored as dict objects)
            print(f"{cards.get('name')} -- {cards.get('code')}", file=mtgfile)  # write the data "we want"
    print(cards)
